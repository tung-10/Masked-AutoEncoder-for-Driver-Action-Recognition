...
class VideoClsDataset(Dataset):
    ...
    def __init__(self,
        ...
        if (mode == 'train'):
            ds = []
            la = []
            for data, label in zip(self.dataset_samples, self.label_array):
                if self.args.data_set in ["Cobot", "Ego-Gesture"]:
                    if int(label) == 0:
                        ds += [data] * 1
                        la += [label] * 1
                    else:
                        ds += [data] * 15
                        la += [label] * 15
                elif self.args.data_set in ["IPN-Hand"]:
                    if int(label) in [0, 1, 2]:
                        ...
                        ds += [data] * 15
                        la += [label] * 15
                elif self.args.data_set in ["UTCDA"]:
                    if int(label) == 0:
                        ds += [data]
                        la += [label]
                    else:
            ...
            for idx in range(len(self.label_array)):
                ...
                # sample_name = os.path.basename(sample).rstrip(".MP4")
                try:
                    num_frames = len(VideoReader(sample, num_threads=1, ctx=cpu(0)))
                except:
                    print("{} broken".format(sample))
                    continue
                for i in range(0, num_frames-1, 30):
                    self.test_label_array.append(sample_label)
                    self.test_dataset.append(sample)
                    self.start_frame_idxs.append(i)
    ...
    def __getitem__(self, index):
        if self.mode == 'train':
            ...
            buffer = self.load_video(sample, start_frame_idx=start_frame_idx)
            while len(buffer) == 0:
    ...
    def load_video(self, sample, start_frame_idx=0, sample_rate_scale=1):
        fname = sample
