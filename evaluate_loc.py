...
class VideoInferDataset(Dataset):
    def __init__(self, video_csv_path, clip_len=16, frame_sample_rate=1, clip_stride=15, short_side_size=224, new_height=224, new_width=224, keep_aspect_ratio=True, crop=False, view="dash"):
        ...
        if self.view == "Ego-gesture":
            data_fixed = []
            for data_path in data_list:
                out_file = data_path[:-4] + "_fixed.mp4"
                if os.path.exists(out_file):
                    print(f"[SKIP] {out_file} đã tồn tại")
                    data_fixed.append(out_file)
                    continue
                    
                # print(f"[FIXING] {in_file} -> {out_file}")
                try:
                    subprocess.run([
                        "ffmpeg", "-i", data_path,
                        "-c:v", "mpeg4", "-q:v", "2",
                    ...
                    print(f"[ERROR] Không thể xử lý {data_path}")
                    print(e.stderr.decode("utf-8"))
            data_list = data_fixed
    ...
    def __getitem__(self, index): 
        ...
        if self.crop:
            buffer = buffer[:, 64:-64, 128:, :]
        buffer = self.data_resize(buffer)
        buffer = self.data_transform(buffer)
        if self.view == "Ego-gesture":
            name = os.path.splitext(sample.split("videos_rgb/")[1])[0].replace("/", "_")
        else: 
            name = sample.split("/")[-1].split(".")[0]
        return buffer, name, t_index
...
def main(args, ds_init):
    ...
    with open(os.path.join(args.output_dir, "utcda_view.pkl"), "wb") as f:
        pickle.dump(vmae_16x4, f)
    # results.to_csv(os.path.join(args.output_dir, "cobot_vmae_16x4.csv"), index=False)
