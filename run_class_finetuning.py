...
def main(args, ds_init):
    ...
    if args.eval:
        preds_file = os.path.join(args.output_dir, str(global_rank) + '___.txt')
        ...
        if global_rank == 0:
            ...
            if args.output_dir and utils.is_main_process():
                with open(
                        os.path.join(args.output_dir, "log___.txt"),
                        mode="a",
                        encoding="utf-8") as f:
    ...
    for epoch in range(args.start_epoch, args.epochs):
        encoder_text = prompt.TextCLIP("RN50", device=device)
        token_text = encoder_text.tokenize_list().to(device)
